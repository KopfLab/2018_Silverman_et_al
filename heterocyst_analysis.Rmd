...
## Visualize half-saturation constant

```{r SI_enzyme_kinetics_model, fig.width = 6, fig.height = 5}
mm_model %>% 
  # add predictions
  mutate(
    data = pmap(list(data = data, Km.bar = Km.bar, slope = slope), 
                function(data, Km.bar, slope) mutate(data, n_cbh_pred = slope*pN2/growth_rate.day * 1/(Km.bar * eps_fix / eps_mean + pN2)))
  ) %>% 
  unnest(data) %>% 
  ggplot(aes(x = pN2)) + 
  # Km estimate
  geom_rect(map = aes(xmin = Km.bar - Km_se.bar, xmax = Km.bar + Km_se.bar, ymin = -Inf, ymax = Inf), fill = "light grey") + 
  geom_vline(map = aes(xintercept = Km.bar), linetype = 2) +
  # model predictions
  geom_smooth(method = "loess", span = 1, se = FALSE, map = aes(y = n_cbh_pred), color = cbPalette[5]) +
  geom_point(map = aes(y = n_cbh_pred), size = 5, shape = 23, color = "black", fill = cbPalette[5]) +
  # original data
  geom_point(map = aes(y = n_cbh_mean), size = 4, shape = 21, color = "black", fill = "gray") + 
  # organism panels
  facet_wrap(~organism, ncol=2) + 
