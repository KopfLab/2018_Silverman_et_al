...
## Summary plot with all stationary phase data points
...
heterocyst_data %>% 
  ...
  # averages
  geom_point(data = calculate_cbh_avgs, map = aes(y = n_cbh.avg), size = 4, shape = 21, color = "black", fill = "white") + 
  # panels
...
## Comparison plot of density distributions across different growth conditions
...
## Visualize means and standard deviations 
...
mm_model <- 
  ...
  left_join(calculate_eps_avgs(isotope_data), by = c("pN2", "organism")) %>% 
  left_join(isotope_model, by = "organism") %>% 
  # run model
  nest(-organism) %>% 
  mutate(
    # use nonlinear least squares fit to find Km
    model = map(data, ~nls(
      n_cbh_mean ~ pN2/growth_rate.day * 1/(Km.bar * eps_fix / eps_avg + pN2) + 0, 
      data = na.omit(.x), start = list(Km.bar = 0.5), algorithm = "plinear")),
    coefficients = map(model, tidy),
    Km.bar = map_dbl(coefficients, ~filter(.x, term == "Km.bar")$estimate),
    Km_se.bar = map_dbl(coefficients, ~filter(.x, term == "Km.bar")$std.error)
  ) 

# print result of the fit
mm_model %>% select(organism, Km.bar, Km_se.bar) %>% kable(d=3)
```
