...
## Determine aqueous/gas fractionation

```{r SI_gas_aq_equilibrium, fig.width = 8, fig.height = 6, warning=FALSE}
# literature data
eq_data <- read_excel(file.path("data", "2017_Silverman_et_al-isotope_literature_data.xlsx"), sheet = "gas liquid equilibrium")
...
eq_regression <- eq_data %>% 
  ...
  mutate(
    ...
    bootstrap = map(data, ~boot(data = .x, R = n_bootstrap,
      statistic = function (x, idx) {
        ...
        m <- lm(eps_aq_gas.permil ~ T.C, data = x[idx,])
  ...
  mutate(
    T.C = 27,
    eps_aq_gas.permil = intercept + T.C * slope,
    eps_aq_gas_se.permil = sqrt(intercept_se^2 + (T.C*slope_se)^2)
  )
...
# visualize data
eq_data %>% 
  ggplot(aes(x = T.C, y = eps_aq_gas.permil, color = source)) +
  # linear fit
  ...
  geom_errorbar(data = eq_regression, color = "black", 
                map = aes(ymin = eps_aq_gas.permil - eps_aq_gas_se.permil, ymax = eps_aq_gas.permil + eps_aq_gas_se.permil)) +
...
model <-
  ...
  merge(select(eq_regression, eps_aq_gas.permil, eps_aq_gas_se.permil)) %>% 
  as_data_frame() %>% 
  mutate(
    ...
    eps_fix = intercept - eps_aq_gas.permil,
    eps_fix_se = sqrt(intercept_se^2 + eps_aq_gas_se.permil^2),
...
```{r figure_isotope_model, fig.width = 8, fig.height = 6, warning=FALSE}
model %>% 
  # unnest 
  unnest(data) %>% 
  # select averages for plotting
  select(x, x_se, eps_mean, eps_mean_se, eps_fix, eps_fix_se, eps_aq_gas.permil) %>% unique() %>% 
  ggplot(aes(x = x, y = eps_mean)) +
  # intercept (eps fix estimate)
  geom_rect(map = aes(ymin = eps_fix - eps_fix_se + eps_aq_gas.permil, 
                      ymax = eps_fix + eps_fix_se + eps_aq_gas.permil, 
                      xmin = -Inf, xmax = Inf), fill = "light grey") + 
  geom_hline(map = aes(yintercept = eps_fix + eps_aq_gas.permil), linetype = 2) +
  # linear fit
  geom_smooth(method = "lm", map = aes(color = NULL), color = "black", se = FALSE, fullrange = TRUE) +
  ...
  scale_y_continuous(breaks = c(0:20)*-0.2, 
                     sec.axis = sec_axis(
                       ~.-eq_regression$eps_aq_gas.permil,
                       breaks = c(0:20)*-0.2,
                       name = TeX("$^{15}\\epsilon_{\\,\\frac{N_{org}}{N_{2,aq}}}$ \\[\U2030\\]"))) + 
  # labels
  labs(y = TeX("$^{15}\\epsilon_{\\,\\frac{N_{org}}{N_{2,gas}}}$ \\[\U2030\\]"), 
       x = TeX("$\\frac{\\mu \\cdot n_{cbh}}{pN_2}\\,\\left[\\frac{cells}{day\\cdot bar}\\right]$")) + 
  # theme
  theme_figure(legend = FALSE, grid = TRUE) + theme(plot.margin = unit(c(0, 0.5, 0, 0), "cm"))
```
